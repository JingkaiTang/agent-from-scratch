...
右侧 Skills 面板展示所有 skill。已加载的 skill 在左侧出现 3px 青色色条，
右上角显示"已加载"标签，背景变为浅青色（按会话作用域）。
未加载 = 纯白 cell，无任何标记——刻意不用 checkbox 形态，避免被误以为是"启用/禁用"开关。
...
3. **触发加载**：发送代码 review 请求，如 `帮我 review 这段代码：def add(a,b): return a-b` → 观察：
   ...
   - 右侧 code-review cell 出现左侧色条 + "已加载"标签
   - LLM 回复符合 SKILL.md 中"总体评价 / 问题 / 建议 / 亮点"结构
4. **不触发**：发送 "今天天气怎么样" → LLM 不会乱加载 skill
5. **会话切换徽章刷新**：在会话 A 加载过后，新建会话 B → "已加载"标签消失；切回 A → 标签恢复
