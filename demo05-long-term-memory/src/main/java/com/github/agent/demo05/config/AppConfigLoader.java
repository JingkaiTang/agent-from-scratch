package com.github.agent.demo05.config;
 ...
 *   <li>{@code longTermMemory.enabled} —— 是否启用长期记忆（默认 true）</li>
 * </ul>
 * <p>
...
public class AppConfigLoader {
    ...
    private static final boolean DEFAULT_LONG_TERM_MEMORY_ENABLED = true;
    ...
    private boolean longTermMemoryEnabled = DEFAULT_LONG_TERM_MEMORY_ENABLED;
    ...
    public AppConfigLoader load() {
        try {
            ...
            // 长期记忆配置
            JsonNode longTermNode = root.path("longTermMemory");
            if (!longTermNode.isMissingNode()) {
                if (longTermNode.has("enabled")) {
                    longTermMemoryEnabled = longTermNode.get("enabled").asBoolean(DEFAULT_LONG_TERM_MEMORY_ENABLED);
                }
            }
            log.info("长期记忆: enabled={}", longTermMemoryEnabled);
    ...
    private void createDefaultConfig() throws IOException {
        ...
        ObjectNode longTermNode = root.putObject("longTermMemory");
        longTermNode.put("enabled", DEFAULT_LONG_TERM_MEMORY_ENABLED);
