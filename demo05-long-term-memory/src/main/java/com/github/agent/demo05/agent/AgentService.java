...
import com.github.agent.demo05.core.AgentCallback;
import com.github.agent.demo05.core.AgentLoop;
import com.github.agent.demo05.config.AppConfigLoader;
import com.github.agent.demo05.llm.LLMClient;
import com.github.agent.demo05.longmem.LongTermMemoryManager;
import com.github.agent.demo05.memory.ConversationCompactor;
import com.github.agent.demo05.memory.MemoryConfig;
import com.github.agent.demo05.memory.MemoryManager;
import com.github.agent.demo05.memory.TokenCounter;
import com.github.agent.demo05.model.ChatMessage;
import com.github.agent.demo05.session.Session;
import com.github.agent.demo05.session.SessionChangeListener;
import com.github.agent.demo05.session.SessionManager;
import com.github.agent.demo05.session.SessionStore;
import com.github.agent.demo05.tool.ToolRegistry;
import com.github.agent.demo05.tool.impl.ExecTool;
...
public class AgentService {
    ...
    public boolean initialize() {
        ...
        MemoryConfig memoryConfig = new MemoryConfig(
                appConfig.getMaxContextTokens(),
                appConfig.getCompactionThreshold(),
                appConfig.getKeepRecentRounds());
        TokenCounter tokenCounter = new TokenCounter();
        ConversationCompactor compactor = new ConversationCompactor(llmClient, tokenCounter);
        MemoryManager memoryManager = new MemoryManager(memoryConfig, tokenCounter, compactor);
        agentLoop.setMemoryManager(memoryManager);
        log.info("记忆管理器已启用: {}", memoryConfig);

        // ⭐ 初始化长期记忆管理器（跨会话的知识持久化和检索）
        longTermMemoryManager = new LongTermMemoryManager(
                llmClient, appConfig.isLongTermMemoryEnabled());
        longTermMemoryManager.init();
        agentLoop.setLongTermMemoryManager(longTermMemoryManager);
        log.info("长期记忆已{}", longTermMemoryManager.isEnabled() ? "启用" : "禁用");
    ...
    public void sendMessage(String message, Consumer<String> onSuccess, Consumer<String> onError) {
        ...
        executor.submit(() -> {
            try {
                ...
                // ⭐ 异步触发长期记忆提取（不阻塞 Agent 回复）
                triggerLongTermMemoryIngest();
    ...
    /**
     * 异步触发长期记忆提取 —— 从当前对话中提取知识写入 memories.md。
     * <p>
     * 不阻塞 Agent 回复，在后台线程中执行。
     * 提取完成后通过回调通知 GUI 展示写入事件。
     */
    private void triggerLongTermMemoryIngest() {
        if (longTermMemoryManager == null || !longTermMemoryManager.isEnabled()) {
            return;
        }

        try {
            List<ChatMessage> history = agentLoop.getConversationHistory();
            int count = longTermMemoryManager.ingest(history);
            if (count > 0) {
                log.info("📝 长期记忆 Ingest 完成: 写入 {} 条", count);
                // 通过回调通知 GUI（需要在 AgentLoop 的 callback 上调用）
                // 注意：这里直接访问 agentLoop 的 callback 不太优雅，
                // 但为了教学简洁性，暂时这样处理
            }
        } catch (Exception e) {
            log.warn("长期记忆 Ingest 异常（不影响主流程）: {}", e.getMessage());
        }
    }
        ...
        try {
            ...
            if (currentHistory.size() > historyBefore) {
                ...
                for (ChatMessage msg : newMessages) {
                    ...
                    // 跳过 system 消息（长期记忆注入的临时上下文，不需要持久化）
                    if ("user".equals(msg.getRole()) || "system".equals(msg.getRole())) {
                        continue;
                    }
