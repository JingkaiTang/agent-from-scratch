...
/**
 * Agent 核心 —— ReAct 循环（支持 SSE 流式响应 + 回调通知 + 记忆管理）。
 * <p>
 * 在 Demo03 的基础上新增短期记忆管理：每次调用 LLM 前自动检查 token 用量，
 * 超过阈值时触发滑动窗口 + LLM 摘要压缩。
 * <p>
